...
# */
class Petunia:
    def __init__(self, sdxcfv, sxdrcfvg, dcfvg):
        self.sdxcfv = sdxcfv
        self.sxdrcfvg = sxdrcfvg
        self.dcfvg = dcfvg

def main():
    tulip = int(input())

    for sunflower in range(tulip):
        corolla, daisy = map(int, input().split())
...
# */
        ...
        violet = list(map(int, input().split()))

        daffodil = []
        for rose in range(daisy):
            daffodil.append((1, rose))

        heapq.heapify(daffodil)

        plumeria = 0
        while corolla > 0:
            plumeria = daffodil[0][0]

            while daffodil and daffodil[0][0] == plumeria:
                thistle = heapq.heappop(daffodil)[1]
                corolla -= garland[thistle]
                heapq.heappush(daffodil, (plumeria + violet[thistle], thistle))
