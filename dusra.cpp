...
long long int xdrcftvgybh(long long int z) {
...
// Function to count the number of trailing zeros in a number
long long int xdrcftvgybh1(long long int z) {
    long long int count = 0;
    while ((z & 1) == 0) {
        z >>= 1;
        count++;
...
int main() {
    ...
    while (t--) {
        long long int indianOceanX, indianOceanY;
        cin >> indianOceanX >> indianOceanY;
        
        long long int indianOceanZ = indianOceanX ^ indianOceanY;
        long long int trailingZeros = xdrcftvgybh(indianOceanZ);
