...
long long int usingBitMask(long long int n, long long int m) {
    long long int iota = 1;
    long long int curentnumber = 0;
    long long int ans = 0;
    ...
    while (n > 0) {
        if (n % 2 != 1) {
            // ans += iota;
            // curentnumber += iota;
            long long int jk = min(iota - curentnumber, curentnumber + 1);
            if (jk <= m) {
                ans += iota;
            }
...
int main() {
    ...
    while(t--) {
        ...
        long long int ans = usingBitMask(n, m);
        cout << ans << endl;
    }
    return 0;
}
