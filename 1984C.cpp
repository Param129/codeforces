...
long long findMaxC(const vector<long long>& mnbgftyh) {
    long long n = mnbgftyh.size();
    long long wsxcfd = 0;
    for (long long num : mnbgftyh) {
        wsxcfd += abs(num);
    }
    vector<vector<bool>> plmhtcvv(n + 1, vector<bool>(2 * wsxcfd + 1, false));
    plmhtcvv[0][wsxcfd] = true; // Base case: c = 0

    for (long long i = 1; i <= n; ++i) {
        for (long long j = 0; j <= 2 * wsxcfd; ++j) {
            if (plmhtcvv[i - 1][j]) {
                plmhtcvv[i][j + mnbgftyh[i - 1]] = true; // Option 1
                plmhtcvv[i][abs(j - mnbgftyh[i - 1])] = true; // Option 2
            }
        }
    }
...
*/
        ...
        vector<long long> plmhtcvv(n + 1), ftvgybh(n + 1);

        int i = 1;
        while (i <= n) {
            ftvgybh[i] = ftvgybh[i - 1] + mnbgftyh[i];
            plmhtcvv[i] = max(plmhtcvv[i - 1] + mnbgftyh[i], abs(ftvgybh[i - 1] + mnbgftyh[i]));
            i++;
        }

        cout << plmhtcvv[n] << endl;
    }
    return 0;
}
